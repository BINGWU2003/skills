...
`excalidraw-diagram` 的预览渲染器依赖 `uv` 和 Playwright。安装 Skill 后可让
Agent 按随附的 [`README.md`](skills/excalidraw-diagram/README.md#setup) 完成首次配置。

`drawio-skill` 导出图片时需要安装 draw.io 桌面版，并确保其 CLI 可调用；
Graphviz 仅在使用可选的自动布局功能时需要。
