...
describe('sync-skills', () => {
  it('parses full, selected, update, help and invalid arguments', () => {
    ...
    expect(parseArgs([], config)).toEqual({ help: false, shouldUpdate: false, skillNames: ['alpha', 'beta'] });
    ...
    expect(() => parseArgs(['missing'], config)).toThrow('没有找到 Skill 配置：missing');
  });

  it('builds submodule arguments with optional remote update', () => {
    expect(buildSubmoduleArgs('/repo', 'sources/alpha', false)).toEqual([
      '-C', '/repo', 'submodule', 'update', '--init', '--', 'sources/alpha',
    ]);
    expect(buildSubmoduleArgs('/repo', 'sources/alpha', true)).toContain('--remote');
  });

  it('allows equal and dotted sibling names but rejects actual traversal', () => {
    const root = path.resolve('repo');
    expect(() => assertInside(root, root, '根目录', { allowEqual: true })).not.toThrow();
    expect(() => assertInside(root, root, '根目录')).toThrow('超出允许范围');
    expect(() => assertInside(root, path.join(root, '..folder'), '目录')).not.toThrow();
  ...
  it('validates SKILL.md and preserves only publishable files', async () => {
    await expect(assertSkillExists('/source', 'alpha', async () => ({ isFile: () => true }))).resolves.toBeUndefined();
    await expect(assertSkillExists('/source', 'alpha', async () => ({ isFile: () => false }))).rejects.toThrow(
      '没有找到 alpha Skill',
    );
    await expect(assertSkillExists('/source', 'alpha', async () => { throw new Error('missing'); })).rejects.toThrow(
      '没有找到 alpha Skill',
    );
  ...
  it('syncs one skill through injected side effects', async () => {
    ...
    await syncSkill('alpha', { submodule: 'sources/project', skillPath: 'skills/alpha' }, true, {
      repoRoot,
      runGitCommand,
      removePath,
      makeDirectory,
    ...
    expect(calls[1]).toEqual(['-C', path.join(repoRoot, 'sources/project'), 'rev-parse', '--short', 'HEAD']);
    expect(ensureSkillExists).toHaveBeenCalledWith(path.join(repoRoot, 'sources/project', 'skills/alpha'), 'alpha');
    expect(removePath).toHaveBeenCalledWith(path.join(repoRoot, 'skills', 'alpha'), { recursive: true, force: true });
    expect(makeDirectory).toHaveBeenCalledWith(path.join(repoRoot, 'skills'), { recursive: true });
    expect(copyPath.mock.calls[0][2].filter('/tmp/.git')).toBe(false);
    expect(logger.log).toHaveBeenCalledWith('已从 alpha@abc1234 同步到 skills/alpha。');
  });

  it('rejects unsafe source and destination configuration', async () => {
    const repoRoot = path.resolve('fixture-repo');
    await expect(syncSkill('alpha', { submodule: '../outside', skillPath: '.' }, false, { repoRoot })).rejects.toThrow(
      '子模块路径超出允许范围',
    );
    await expect(syncSkill('alpha', { submodule: 'sources/project', skillPath: '../../outside' }, false, {
      repoRoot,
    })).rejects.toThrow('Skill 来源路径超出允许范围');
    await expect(syncSkill('..', { submodule: 'sources/project', skillPath: '.' }, false, { repoRoot })).rejects.toThrow(
      '发布目标路径超出允许范围',
    );
  });
  ...
  it('prints help or invokes selected skills through main', async () => {
    const logger = { log: vi.fn() };
    const sync = vi.fn();
    const config = { alpha: { value: 1 }, beta: { value: 2 } };
  ...
  it('reads configuration and uses the default sync adapter', async () => {
    const readTextFile = vi.fn().mockResolvedValue('{"alpha":{"submodule":"sources/a","skillPath":"."}}');
    const runGitCommand = vi.fn(args => args.includes('rev-parse') ? 'deadbee' : '');
    const logger = { log: vi.fn() };
    await main(['alpha'], {
    ...
    expect(logger.log).toHaveBeenCalledWith('已从 alpha@deadbee 同步到 skills/alpha。');
  });
